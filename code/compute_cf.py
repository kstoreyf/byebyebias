...
def counts_cf_proj_auto(cat, rbins, r_cont, proj, qq=False):
    cosmo = 1 #doesn't matter bc passing cz, but required

    ra, dec, cz = cat.T

    proj_type, nprojbins, projfn = get_proj_parameters(proj, rbins=rbins)
...
def counts_cf_proj_cross(data, random, rbins, r_cont, proj):
    cosmo = 1 #doesn't matter bc passing cz, but required
...
def counts_cf_proj(data, random, rbins, r_cont, proj):
...
def compute_cf_proj(dd, dr, rr, qq, nd, nr, rbins, r_cont, proj):
